...
if { [catch { 
	...
	ShowMessage "------------开始编辑时间组规则：$parameter------------"
	if {$expectresult==0} {
	ShowMessage "------------预期结果为：编辑时间组失败------------------"		
	} else {
	ShowMessage "------------预期结果为：编辑时间组成功------------------"		
	}
	set TimeEditRet [TimeEdit $parameter]
	if {[lindex $TimeEditRet 0]==0} {
		ShowMessage "------------实际结果为：编辑时间组失败------------------"		
		} else {
		ShowMessage "------------实际结果为：编辑时间组成功------------------"		
		}
	
	#结果判断
        if {[lindex $TimeEditRet 0]!=$expectresult} {
